...
public class ExperimentsAllTopicsLocal {
	...
	pacts1.addAll(avg1);
		...
		paramsLLDAMSG.add(new LLDAParameters(5, 0.01, 1000,0));
		paramsLLDAMSG.add(new LLDAParameters(8, 0.01,1000,0));
		paramsLLDAMSG.add(new LLDAParameters(10, 0.01, 1000,0));
//		paramsLLDAMSG.add(new LLDAParameters(150, 0.01, 1000,0));
//		paramsLLDAMSG.add(new LLDAParameters(20, 0.01, 2000,0));
//		paramsLLDAMSG.add(new LLDAParameters(50, 0.01,  2000,0));
//		paramsLLDAMSG.add(new LLDAParameters(100, 0.01,2000,0));
//		paramsLLDAMSG.add(new LLDAParameters(150, 0.01, 2000,0));
...
//		paramsHDPUP.add(new HDPParameters(1.0, 0.1, 1.0, 10,3));
...
//		paramsHDPHASH.add(new HDPParameters(1.0, 0.1, 1.0, 10,3));
		...
		strategyParamsHDP.put(TopicModelTrainStrategy.getName(TopicModelTrainStrategy.UP_STRATEGY), paramsHDPMSG);
...
//	}
		...
		//userDataMap=StoreTestData.loadTestUserData("/home/efi/master-thesis/all_userdata");
		for(String name : usersStrictMap.keySet()){
			UserData uData=userDataMap.get(name);
			Collections.sort(uData.getTestTweets());
			refDateMap.put(uData.getUserName(),uData.getTestTweets().get(0).getThinTweet().getTimeStamp());
			//ModelsUtilities.printTestSetStatistics(uData);
		}
//		
		...
		for (ModelInfoSource type : types) {
			...
			for (int i = 0; i < pacts.size(); i++){
				String ct=usersStrictMap.get(pacts.get(i).getUserName());
				if(ct!=null){
				if(ct.equals("top_new")){
			...
			System.out.println(ModelInfoSource.name(type));
		//ExperimentsLLDA.run(ModelInfoSource.name(type), userDataMap,
			...
			//ExperimentsUnsupTopic.runPerUser(ModelInfoSource.name(type), userDataMap, allParamsMap, mergeStrategies,
